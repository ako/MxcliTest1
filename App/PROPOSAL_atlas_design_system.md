...
| **P1** Building Blocks **instantiate** (`USE BUILDING BLOCK Mod.Name [as prefix_]`) | ✅ **fixed** | v1 deep-copy works on the **default `modelsdk` engine** (the skill's "legacy-only today" note is now stale). `use building block Atlas_Web_Content.Card as cust_` expands to the block's real tree — `cust_container2` + `cust_text22` carrying the `card-title` class **and** the nested `Spacing` design property — and `mx check` = 0 errors. ~~Constraint: expands only at page-body top level~~ **now also expands inside a nested `container`** (fixed — see next row). A block has no params, so bind data/text afterwards with `alter page` (or use rebind overrides, below). The recipe library can now be a thin adapter over native blocks |
| **P1** fragment / building-block / slot expansion **inside a nested `container`** | ✅ **fixed** | The constraint I reported last session is resolved: `use building block …`, `use fragment …`, and `slot` now expand at **any depth**, not just page/snippet top level. Verified — `use building block Atlas_Web_Content.Card as nb_` inside `container outerHost { … }` expands the Card *inside* that container (`outerHost → nb_container2 → nb_text22`), `mx check` = 0 errors (previously errored *"unsupported widget type: USE_BUILDING_BLOCK"*) |
| **P1** fragment **parameter bindings** (`datasource` / `action` params) | ✅ **fixed (experimental)** | Closes the "vary *data/behavior*, not just structure" half of parameterized fragments. `define fragment DataPanel($data: datasource, $onEdit: action) as { … listview (datasource: $data){ slot content; actionbutton (action: $onEdit) } }`, used as `use fragment DataPanel ($data: database Travel.Trip, $onEdit: microflow Travel.ACT_Startup) { … }`. Verified: `$data` → `database from Travel.Trip`, `$onEdit` → `microflow Travel.ACT_Startup`, slot payload spliced in, **no `$param` left after expansion**, `mx check` = 0 errors. Every declared param must be supplied; unknown args / type mismatches error. Remaining v1.1: **scalar** params (vary a leaf label/attribute *value*) |
| **P1** building-block **rebind overrides** (`use building block … (datasource:…, action:…)`) | ✅ **fixed (prototype)** | Since blocks are parameterless, a `use building block` now accepts inline overrides that rewrite the copy's **outermost datasource** and **first button** — the "configure at instantiation" ergonomics I flagged as the v1.1 wish. Verified on `Atlas_Web_Content.List_Cards`: `(datasource: database Travel.Trip, action: microflow Travel.ACT_Startup) as tl_` rebound the block's gallery (`tl_gallery1` → `database from Travel.Trip`) and its first `linkbutton` (`tl_actionButton6` → `microflow Travel.ACT_Startup`), `mx check` = 0 errors. Binding-point rule is positional (first datasource widget / first button); for anything more specific, copy with `as prefix_` + `alter page … on prefix_widget` |
...
| **P1** content-slot fragments (`slot` + `use fragment X { … }` payload) | ✅ **fixed** | A fragment can now be a **reusable shell that wraps caller-supplied content** — declare `slot [name]` where the payload lands, then fill it with the `use fragment Card { …widgets… }` form. Verified on the default engine: an Atlas-Card shell + a 2-widget payload expands to the wrapped tree (`cardWrap → cardBody → cardHeading + cardText`, slot marker fully resolved, compound `Card style` prop preserved), `mx check` = 0 errors. Edge cases behave: empty payload → valid empty shell; payload to a fragment with no `slot` → clear error (*"fragment X does not declare a `slot`…"*). This is the composition primitive the recipe library needs before parameterized fragments — a card/panel/section recipe now ships as one slotted fragment. Still v1.1: scalar params for varying a leaf **value** (label/attr) rather than a subtree |

Still open from the table above: Building-Block **author** (`CREATE BUILDING BLOCK`); **scalar** fragment
params (vary a leaf *value* — label/attribute — via `define fragment F($label, $attr)`; the `datasource`/`action`
param kinds and the content slot have landed); richer building-block rebinds beyond the positional first-datasource /
first-button rule; chart colourway; typed `designproperties` authoring ergonomics; lint rules. *(The nested-container
expansion constraint and the "configure at instantiation" wish from prior waves are now **resolved** — see the table.)*
